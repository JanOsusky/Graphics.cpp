...
void initControlPointsDown(S_Vector** points, int offset_x, int offset_y) {
	...
	initControlPointsUp(points, offset_x, offset_y);
		...
		*points = vecCreateEmpty(sizeof(Point2d));
		...
		// Define each control point and add it to the vector using the point2d_vecPushBack() function.
		Point2d p;
		p.x = 0;   p.y = 0;    point2d_vecPushBack(*points, p);
		...
		// Define an alternative coordinate for this control point to ensure C1 continuity.
		// p.x = 210; p.y = -180; point2d_vecPushBack(*points, p);
		p.x = 200 + (40.0 / 250) * 180; p.y = 180; point2d_vecPushBack(*points, p);

		// Define an alternative coordinate for this control point to ensure C1 continuity.
		// p.x = 350; p.y = -180; point2d_vecPushBack(*points, p);
		p.x = 360 - (30.0 / 120) * 180; p.y = 180; point2d_vecPushBack(*points, p);
		...
		// Define an alternative coordinate for this control point to ensure C1 continuity.
		// p.x = 470; p.y = -70;  point2d_vecPushBack(*points, p);
		p.x = 460 + (30.0 / 120) * 70; p.y = 70;  point2d_vecPushBack(*points, p);

		// Define an alternative coordinate for this control point to ensure C1 continuity.
		// p.x = 525; p.y = -70;  point2d_vecPushBack(*points, p);
		p.x = 535 - (10.0 / 40) * 70; p.y = 70;  point2d_vecPushBack(*points, p);
		...
		// Apply the specified offset to each point in the vector using the addPoint2d() function.
...
void	bezierCubicsTrajectory(int precision, const S_Vector* control_points, S_Vector* trajectory_points) {
	...
	for (int i = 0; i < point2d_vecSize(control_points) - 1;) {
		...
		bezierCubic(P1, P2, P3, P4, quality, trajectory_points);
	}
